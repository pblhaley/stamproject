...
export function RecentPurchasesBadge({
  ...
  showThreshold = 1,
  refreshInterval = 0,
  timePeriod = "24h", // Added timePeriod with default
  ...
  const fetchPurchaseCount = useCallback(async () => {
    ...
    try {
      const response = await fetch(`/api/bigcommerce/recent-purchases?productId=${productId}&period=${timePeriod}`)
  ...
  }, [productId, timePeriod]) // Added timePeriod to dependencies
  ...
  const displayMessage = (message || defaultMessages[timePeriod]).replace("{count}", count.toString())
